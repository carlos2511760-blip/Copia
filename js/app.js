(() => {
  ...
  async function deleteProject(id) { const project = data.projects.find(item => item.id === id); if (!project) return; if (!window.confirm(`Excluir definitivamente o projeto “${project.title || 'Sem título'}”?`)) return; const button = $(`[data-delete-project="${CSS.escape(id)}"]`); if (project.dbId && supabase.url && supabase.anonKey) { button.disabled = true; const response = await fetch(apiUrl(`fecart_projects?id=eq.${encodeURIComponent(project.dbId)}`), { method: 'DELETE', headers: apiHeaders() }); if (!response.ok) { button.disabled = false; $('#editor-status').textContent = 'Não foi possível excluir no Supabase. Verifique as políticas do banco.'; return; } } data.projects = data.projects.filter(item => item.id !== id); localStorage.removeItem(`fecart-liked-project-${id}`); persistDraft(); renderAll(); renderEditor(); $('#editor-status').textContent = project.dbId ? 'Projeto excluído do banco de dados e da página.' : 'Projeto excluído do rascunho local.'; }
